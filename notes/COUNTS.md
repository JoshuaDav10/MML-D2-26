...
|---|---|---|
| `F .text` in all rock_neo `.o` | **522** | Functions currently split into C TUs. **NOT the mission denominator.** |
| Functions still unsplit (raw asm) | **597** | `glabel`s in `asm/rock_neo/*.s`, never split into a C TU. |
| **Game functions in ROCK_NEO.EXE** | **1119** | 522 + 597. The MAIN-EXE denominator. |
...
| Matched — ENGINE (real C in tree) | **363** | 522 − 159. `census.py --matched`, authoritative for the main exe. |
...
| **Matched — WHOLE GAME** | **375** | 363 engine + 12 stage. |
...
| By function count (C-mapped slice) | **69.5%** | 363 / 522 — the number historically quoted. Overstates the mission. |
| By function count (main exe) | **32.4%** | 363 / 1119. |
| By function count (stage realm) | **0.2%** | 12 / ~7064. |
| **By function count (WHOLE GAME)** | **~4.3%** | 375 / ~8629. The honest number. |

## Splitting is not progress
A phase-0 split MOVES a function from the unsplit bucket into the C-mapped bucket. It
raises `522` and lowers `597` and leaves **1119 unchanged**. Only a rise in
**matched** is progress.

## Always name which denominator
The 2026-07-26 audit found this project had reported `matched/C-mapped` for months while
597 functions sat in `asm/rock_neo/*.s`, linked by `rock_neo.ld` and invisible to
