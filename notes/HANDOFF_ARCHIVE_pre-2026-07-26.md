# ARCHIVE — HANDOFF.md as it stood before the 2026-07-26 audit rewrite

Preserved verbatim, not edited. This is the accreted per-session banner stack that
HANDOFF.md had become (608 lines). It is kept because the 2026-07-26 audit found that
retro-editing dated entries had already destroyed part of the audit trail of the earlier
count-inflation event — so history gets appended to and superseded, never overwritten.

Counts quoted below are historical and mostly stale. The authoritative live numbers are
in HANDOFF.md and are regenerated by `tools/check_docs.sh`.
