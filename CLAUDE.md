...
## The function map
`notes/FUNCTION_MAP.md` (generated by `tools/gen_map.py`, gated by `check_docs.sh`) is the
single inventory: every function in the game, exactly once, with a state
(MATCHED / STUB / UNSPLIT / STAGE). Query it instead of grepping:
`tools/gen_map.py --query UNSPLIT --max-insns 20 --leaf`.

## Build landmines (2026-07-26 — both cost a broken build)
- **splat offsets:** `rom_offset = vram - 0x8000F800`, not `- 0x80010000`. Use
  `tools/phase0_split.py`, which has the correct constant. Splat is now safe to re-run.
- **Phantom matches:** a `.c` can compile to a `.o` that `rock_neo.ld` never references —
  census counts it, the binary links the raw asm, and the full hash stays OK because the
  ORIGINAL bytes shipped. Guards are in `census.py` / `gen_map.py`. **A green hash does
  not prove your C is in the binary.**
- `tools/audit_count.sh` deletes the overlay BINs (clean rebuild). Run
  `make CPP=cpp chunks` before pushing or `check_overlays` fails.
